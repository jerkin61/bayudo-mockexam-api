...
use Jerquin\Database\Models\Invoice;
use Jerquin\Database\Repositories\InvoiceRepository;
...
class InvoiceController extends CoreController
{
    ...
    public function index(Request $request)
  {
    ...
    if ($request->has('invoiceSearch')&& $request->invoiceSearch !== 'undefined') {
        $invoiceNumber = $request->input('invoiceSearch');
        $query->where('invoice_number', 'like', '%' . $invoiceNumber . '%');
    }
   if ($request->status === 'existing' || $request->status === 'hold') {
        $status = $request->input('status');
        $query->where('status', '=', $status);
    };
    
    if ($request->has('date') && $request->date !== 'undefined' && $request->date !== 'null') {
        $date = $request->input('date');
        $query->whereDate('date_of_invoice', '=', $date);
    }
    $totalSum = $query->sum('total');

    $results = $query->paginate($limit)->withQueryString()->toArray();
    $results['total_sum'] = $totalSum;
